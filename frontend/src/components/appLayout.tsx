...
import { LayoutDashboard, FolderGit2, Menu, X } from 'lucide-react'
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-black text-zinc-50 font-sans selection:bg-white selection:text-black">
      ...
      <div className="md:hidden sticky top-0 z-50 bg-zinc-950 border-b border-zinc-900 flex items-center justify-between p-4">
        ...
        <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="p-2 bg-zinc-900 border border-zinc-800 rounded text-zinc-400 hover:text-white">
          {mobileMenuOpen ? <X size={20} /> : <Menu size={20} />}
        </button>
      </div>

      {/* Sidebar */}
      <aside className={`fixed md:sticky top-0 left-0 h-screen w-64 bg-zinc-950 border-r border-zinc-900 z-40 transform transition-transform duration-200 ease-in-out ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'} flex flex-col`}>
          ...
          {navItems.map(item => {
            ...
            return (
              <Link 
                ...
                className={`flex items-center gap-3 px-4 py-3 text-sm font-mono uppercase tracking-widest transition-colors ${isActive ? 'bg-white text-black font-bold' : 'text-zinc-400 hover:text-white hover:bg-zinc-900'}`}
              >
                <item.icon size={16} />
                {item.name}
              </Link>
